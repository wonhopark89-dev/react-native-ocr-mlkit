...
import {ProcessImageNavigationProps, ProcessImageRouteProps} from '../navigation/Navigator';
import {recognizeImage, Response} from '../mlkit';
import {ResponseRenderer, ResponseRendererText} from '../components/ResponseRenderer';
...
export const ProcessImageScreen = ({route}: ProcessImageScreenProps) => {
  ...
  return (
    <ScrollView style={{flex: 1}}>
      <Image source={{uri}} style={{width: windowWidth, height: windowWidth * aspectRatio}} resizeMode="cover" />
      {!!response && <ResponseRenderer response={response} scale={windowWidth / response.width} />}
      {!!response && <ResponseRendererText response={response} scale={windowWidth / response.width} />}
    </ScrollView>
  );
};
